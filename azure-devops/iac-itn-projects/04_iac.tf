locals {
  ...
  base_iac_variables = {
    ...
    #PLAN
    TF_AZURE_SERVICE_CONNECTION_PLAN_NAME_DEV  = data.azuredevops_serviceendpoint_azurerm.dev_azurerm_iac_plan_service_conn.service_endpoint_name
    TF_AZURE_SERVICE_CONNECTION_PLAN_NAME_UAT  = data.azuredevops_serviceendpoint_azurerm.uat_azurerm_iac_plan_service_conn.service_endpoint_name
    TF_AZURE_SERVICE_CONNECTION_PLAN_NAME_PROD = data.azuredevops_serviceendpoint_azurerm.prod_azurerm_iac_plan_service_conn.service_endpoint_name

    #APPLY
    TF_AZURE_SERVICE_CONNECTION_APPLY_NAME_DEV  = data.azuredevops_serviceendpoint_azurerm.dev_azurerm_iac_deploy_service_conn.service_endpoint_name
    TF_AZURE_SERVICE_CONNECTION_APPLY_NAME_UAT  = data.azuredevops_serviceendpoint_azurerm.uat_azurerm_iac_deploy_service_conn.service_endpoint_name
    TF_AZURE_SERVICE_CONNECTION_APPLY_NAME_PROD = data.azuredevops_serviceendpoint_azurerm.prod_azurerm_iac_deploy_service_conn.service_endpoint_name
  }
...
module "iac_code_review" {
  ...
  github_service_connection_id = data.azuredevops_serviceendpoint_github.azure_devops_github_pr.id
  ...
  variables = merge(
    ...
    contains(each.value.envs, "d") && try(each.value.kv_name, "") != "" ? {
      tf_dev_aks_apiserver_url         = module.dev_secrets[each.value.name].values["${local.aks_dev_platform_name}-apiserver-url"].value,
      tf_dev_aks_azure_devops_sa_cacrt = module.dev_secrets[each.value.name].values["${local.aks_dev_platform_name}-azure-devops-sa-cacrt"].value,
      tf_dev_aks_azure_devops_sa_token = base64decode(module.dev_secrets[each.value.name].values["${local.aks_dev_platform_name}-azure-devops-sa-token"].value),
    } : {},
    contains(each.value.envs, "u") && try(each.value.kv_name, "") != "" ? {
      tf_uat_aks_apiserver_url         = module.uat_secrets[each.value.name].values["${local.aks_uat_platform_name}-apiserver-url"].value,
  ...
  service_connection_ids_authorization = [
    ...
    data.azuredevops_serviceendpoint_azurerm.dev_azurerm_iac_plan_service_conn.id,
    data.azuredevops_serviceendpoint_azurerm.uat_azurerm_iac_plan_service_conn.id,
  ]
}
...
module "iac_deploy" {
  ...
  service_connection_ids_authorization = [
    data.azuredevops_serviceendpoint_github.azure_devops_github_ro.id,
    data.azuredevops_serviceendpoint_azurerm.dev_azurerm_iac_plan_service_conn.id,
    data.azuredevops_serviceendpoint_azurerm.uat_azurerm_iac_plan_service_conn.service_endpoint_id,
    data.azuredevops_serviceendpoint_azurerm.prod_azurerm_iac_plan_service_conn.service_endpoint_id,

    data.azuredevops_serviceendpoint_azurerm.dev_azurerm_iac_deploy_service_conn.service_endpoint_id,
    data.azuredevops_serviceendpoint_azurerm.uat_azurerm_iac_deploy_service_conn.service_endpoint_id,
    data.azuredevops_serviceendpoint_azurerm.prod_azurerm_iac_deploy_service_conn.service_endpoint_id,
  ]
}
