...
locals {
  # global vars
  idpay-payment-variables = {
    dockerfile = "Dockerfile"
  }
  # global secrets
  idpay-payment-variables_secret = {

  }
  # code_review vars
  idpay-payment-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.idpay-payment.repository.organization
    sonarcloud_project_key  = "${var.idpay-payment.repository.organization}_${var.idpay-payment.repository.name}"
    sonarcloud_project_name = var.idpay-payment.repository.name
  }
  # code_review secrets
  idpay-payment-variables_secret_code_review = {

  }
  # deploy vars
  idpay-payment-variables_deploy = {
    K8S_IMAGE_REPOSITORY_NAME        = replace(var.idpay-payment.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.idpay-payment.repository.name
  ...
  # deploy secrets
  idpay-payment-variables_secret_deploy = {

  }
}

module "idpay-payment_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v5.2.0"
  count  = var.idpay-payment.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.idpay-payment.repository
  github_service_connection_id = local.service_endpoint_io_azure_devops_github_pr_id
  path                         = var.idpay-payment.pipeline.path

  ci_trigger_use_yaml           = true
  pull_request_trigger_use_yaml = true

  variables = merge(
    local.idpay-payment-variables,
    local.idpay-payment-variables_code_review,
  )

  variables_secret = merge(
    local.idpay-payment-variables_secret,
    local.idpay-payment-variables_secret_code_review,
  )
...
module "idpay-payment_deploy" {
  ...
  variables = merge(
    local.idpay-payment-variables,
    local.idpay-payment-variables_deploy,
  )

  variables_secret = merge(
    local.idpay-payment-variables_secret,
    local.idpay-payment-variables_secret_deploy,
  )
