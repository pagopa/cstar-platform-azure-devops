variable "multi_initiative_merchants_catalog_frontend" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "idpay-merchants-catalog-portal"
      branch_name     = "refs/heads/main"
      pipelines_path  = ".devops"
      yml_prefix_name = null
    }
    pipeline = {
      path = "idpay\\idpay-merchants-catalog-portal"
    }
  }
}
...
locals {
  ...
  # deploy vars
  multi_initiative_merchants_catalog_frontend_variables_deploy = {
...
module "multi_initiative_merchants_catalog_frontend_deploy" {
  ...
  repository                   = var.multi_initiative_merchants_catalog_frontend.repository
  github_service_connection_id = local.service_connection_github_ro_id


  pipeline_name         = "${local.product}-multi-initiative-merchants-catalog.deploy"
  pipeline_yml_filename = "multi-initiative-deploy-pipelines.yml"
  path                  = var.multi_initiative_merchants_catalog_frontend.pipeline.path

  ci_trigger_enabled  = true
  ci_trigger_use_yaml = true

  variables = merge(
    local.multi_initiative_merchants_catalog_frontend_variables_deploy,
  )
