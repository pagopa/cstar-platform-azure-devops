...
# module "federated_cert_pipeline_idpay_dev" {
#   source = "./.terraform/modules/__devops_v0__/azuredevops_build_definition_tls_cert_federated"

#   for_each = { for k, v in local.certificates_idpay : k => v if v.env == "dev" }
...
#   dns_zone_name           = local.env_mappings_idpay[each.value.env].dns_zone_name
#   dns_zone_resource_group = local.env_mappings_idpay[each.value.env].dns_zone_rg
#   tenant_id               = data.azurerm_client_config.current.tenant_id
#   subscription_name       = local.env_mappings_idpay[each.value.env].subscription_name
#   subscription_id         = local.env_mappings_idpay[each.value.env].subscription_id

#   credential_key_vault_name           = local.env_mappings_idpay[each.value.env].credential_key_vault_name
#   credential_key_vault_resource_group = local.env_mappings_idpay[each.value.env].credential_key_vault_resource_group

#   variables = merge(
#     local.env_mappings_idpay[each.value.env].variables,
#     each.value.variables
#   )

#   variables_secret = merge(
#     local.env_mappings_idpay[each.value.env].variables_secret,
#     each.value.variables_secret
#   )

#   service_connection_ids_authorization = [local.env_mappings_idpay[each.value.env].service_endpoint]
...
# module "federated_cert_pipeline_idpay_uat" {
#   source = "./.terraform/modules/__devops_v0__/azuredevops_build_definition_tls_cert_federated"

#   for_each = { for k, v in local.certificates_idpay : k => v if v.env == "uat" }
...
# module "federated_cert_pipeline_idpay_prod" {
#   source = "./.terraform/modules/__devops_v0__/azuredevops_build_definition_tls_cert_federated"

#   for_each = { for k, v in local.certificates_idpay : k => v if v.env == "prod" }
