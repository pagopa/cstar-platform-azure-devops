...
module "api-io-cstar-uat-pagopa-it-cert_az" {
  ...
  name       = "${var.api-io-cstar-uat-pagopa-it.pipeline.dns_record_name}.${var.api-io-cstar-uat-pagopa-it.pipeline.dns_zone_name}"
  #tfsec:ignore:general-secrets-no-plaintext-exposure
  #tfsec:ignore:GEN003
  ...
  credential_subcription              = local.api-io-cstar-uat-pagopa-it.credential_subcription
  ...
  schedules = {
    ...
    branch_filter = {
      include = [var.api-io-cstar-uat-pagopa-it.repository.branch_name]
      exclude = []
    }
  }
