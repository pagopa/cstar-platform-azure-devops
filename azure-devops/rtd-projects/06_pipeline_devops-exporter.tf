variable "rtd-ms-exporter" {
  default = {
    repository = {
      organization    = "pagopa"
      name            = "rtd-ms-exporter"
...
locals {
  # global vars
  rtd-ms-exporter-variables = {
    dockerfile = "Dockerfile"
  }
  # global secrets
  rtd-ms-exporter-variables_secret = {

  }
  # code_review vars
  rtd-ms-exporter-variables_code_review = {
    sonarcloud_service_conn = "SONARCLOUD-SERVICE-CONN"
    sonarcloud_org          = var.rtd-ms-exporter.repository.organization
    sonarcloud_project_key  = "${var.rtd-ms-exporter.repository.organization}_${var.rtd-ms-exporter.repository.name}"
    sonarcloud_project_name = var.rtd-ms-exporter.repository.name
  }
  # code_review secrets
  rtd-ms-exporter-variables_secret_code_review = {

  }
  # deploy vars
  rtd-ms-exporter-variables_deploy = {
    K8S_IMAGE_REPOSITORY_NAME        = replace(var.rtd-ms-exporter.repository.name, "-", "")
    DEPLOY_NAMESPACE                 = local.domain
    SETTINGS_XML_RW_SECURE_FILE_NAME = "settings-rw.xml"
    SETTINGS_XML_RO_SECURE_FILE_NAME = "settings-ro.xml"
    HELM_RELEASE_NAME                = var.rtd-ms-exporter.repository.name
  ...
  # deploy secrets
  rtd-ms-exporter-variables_secret_deploy = {

  }
}

module "rtd-ms-exporter_code_review" {
  source = "git::https://github.com/pagopa/azuredevops-tf-modules.git//azuredevops_build_definition_code_review?ref=v5.2.0"
  count  = var.rtd-ms-exporter.pipeline.enable_code_review == true ? 1 : 0

  project_id                   = data.azuredevops_project.project.id
  repository                   = var.rtd-ms-exporter.repository
  github_service_connection_id = local.service_endpoint_io_azure_devops_github_pr_id
  path                         = var.rtd-ms-exporter.pipeline.path

  pull_request_trigger_use_yaml = true

  variables = merge(
    local.rtd-ms-exporter-variables,
    local.rtd-ms-exporter-variables_code_review,
  )

  variables_secret = merge(
    local.rtd-ms-exporter-variables_secret,
    local.rtd-ms-exporter-variables_secret_code_review,
  )
...
module "rtd-ms-exporter_deploy" {
  ...
  variables = merge(
    local.rtd-ms-exporter-variables,
    local.rtd-ms-exporter-variables_deploy,
  )

  variables_secret = merge(
    local.rtd-ms-exporter-variables_secret,
    local.rtd-ms-exporter-variables_secret_deploy,
  )
