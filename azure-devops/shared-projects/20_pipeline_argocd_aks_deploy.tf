variable "srtp_aks_deploy" {
  default = {
    repository = {
      organization   = "pagopa"
      name           = "cstar-shared-deploy-aks"
      branch_name    = "refs/heads/main"
      pipelines_path = ".devops"
    }
    pipeline = {
      path = "argocd"
      name = "shared-deploy-aks"
    }
  }
}
...
locals {
  # global vars
  aks_deploy_variables = {}

  # global secrets
  aks_deploy_variables_secret = {}
  ...
  # deploy secrets vars
  aks_deploy_variables_secret_deploy = {
...
module "srtp_aks_deploy" {
  ...
  variables = merge(
    local.aks_deploy_variables,
    local.aks_deploy_variables_deploy
  )

  variables_secret = merge(
    local.aks_deploy_variables_secret,
    local.aks_deploy_variables_secret_deploy
  )
